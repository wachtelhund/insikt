...
def _render(graph, now):
    ...
    return render_report(graph, meta={"frameworks": ["hermes"], "snapshot_id": 1, "host": "test-host"}, hygiene=hygiene, now=now)
...
def test_report_embedded_json_is_valid(hermes_graph, now):
    ...
    assert payload["meta"]["host"] == "test-host"
...
def test_report_includes_hygiene_and_cost(hermes_graph, now):
    ...
    assert payload["cost"]["total_tokens"] == 3000
